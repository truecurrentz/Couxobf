...
| 14 | Decoy constants and instructions | 🔶 | **Partly built.** `decoys`/`decoy_constants` are live: the pool plants unreferenced entries as it interns (a maximum build of `maze.luau` carries 24), derived from values already in the pool, scattered rather than tacked on, and the report prints the count. The dispatch-chain half is not: `junk_level` is declared and inert, and is now in the pending list for exactly that reason. So the decoys are constants only. |
...
| 46 | Integrity checks at multiple semantic boundaries | 🔶 | At build time: one AEAD tag per pool, per bank and per payload, plus header/opcode-count/entry-start validation of every prototype against its own group's format (`validate_proto`, `validate_module`), so an encoder/validator disagreement fails the build instead of shipping. At run time: the tag, and the guard's re-check of the dump surfaces at every VM entry. Not there: a per-block or per-instruction check - `self_test` and `integrity_level` stay declared and inert. |
...
| 41 | Interleave decoding with execution | 🔶 | True for strings (per-occurrence tickets, resolved lazily) and for the pool's plaintext (one constant materialized per read). Not true for the pool blob or the VM payload, both opened in full on first use: `chunking_level`, `chunk_size` and `lazy_decode` are declared and inert, and the report names them as such. Per-chunk keys plus lazy chunk opening are the gap, and the runtime half - chunk table, per-chunk offsets, the lifetime policy - is where the work is. |
...
| Constant representation | 🔶 — pool layout keyed, decoys planted, plaintext materialized per read; chunking and per-chunk keys are inert (#14, #41) |
...
| VM / native boundary | ⬜ — `mixed_execution` is declared and pending; a function is wholly native or wholly virtual (#80) |
...
```
"junk_level" -- that field exists but the pipeline does not read it yet, and this
